...
import type { ChatsResponse, MessagePageResponse, SendMessageResponse } from "../src/models/chat";
...
describe("testApi Unit Tests", () => {
	...
	test("apiLogin success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true } as Response);
		const result = await apiLogin({ email: "test@test.com", password: "Password123" });
	...
	test("apiSignUp success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true } as Response);
		const result = await apiSignUp({ email: "test@test.com", first_name: "Test", last_name: "User", password: "Password123" });
		expect(result.status).toBe(200);
	});
	...
	test("apiCurrent not ok", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 401, ok: false } as Response);
		const result = await apiCurrent();
	...
	test("apiChats success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => mockChatsResponse } as Response);
	...
	test("apiMessages success with timestamp processing", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => mockMessagePageResponse } as Response);
		const result = await apiMessages({ chat_session_id: 1, message_id: null });
		expect(result.status).toBe(200);
		expect(result.result?.message_page[0].timestamp).toBe("2025-01-01T10:00:00Z");
	});

	test("apiMessages not ok", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 404, ok: false } as Response);
		const result = await apiMessages({ chat_session_id: 1, message_id: null });
	...
	test("apiSendMessage success with timestamp processing", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => mockSendMessageResponse } as Response);
		const result = await apiSendMessage({ chat_session_id: 1, text: "Hello", itinerary_id: null });
		expect(result.status).toBe(200);
		expect(result.result?.bot_message.timestamp).toBe("2025-01-01T10:00:00Z");
	});

	test("apiSendMessage not ok", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 400, ok: false } as Response);
		const result = await apiSendMessage({ chat_session_id: 1, text: "Hello", itinerary_id: null });
	...
	test("apiNewChatId success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => ({ chat_session_id: 123 }) } as Response);
	...
	test("apiItineraryDetails success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => mockItinerary } as Response);
	...
	test("apiGetSavedItineraries success", async () => {
		vi.mocked(customFetch).mockResolvedValue({ status: 200, ok: true, json: async () => mockSavedItinerariesResponse } as Response);
