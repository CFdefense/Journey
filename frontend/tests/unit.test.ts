...
describe("Itinerary Helper Tests", () => {
	...
	test("Test getTimeBlockFromTimestamp - Morning", () => {
		expect(getTimeBlockFromTimestamp("2025-01-01T08:00:00")).toBe(
			"Morning"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T04:00:00Z")).toBe(
			"Morning"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T11:59:59Z")).toBe(
			"Morning"
		);
	});

	test("Test getTimeBlockFromTimestamp - Afternoon", () => {
		expect(getTimeBlockFromTimestamp("2025-01-01T12:00:00")).toBe(
			"Afternoon"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T15:30:00Z")).toBe(
			"Afternoon"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T17:59:59Z")).toBe(
			"Afternoon"
		);
	});

	test("Test getTimeBlockFromTimestamp - Evening", () => {
		expect(getTimeBlockFromTimestamp("2025-01-01T18:00:00")).toBe(
			"Evening"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T22:00:00Z")).toBe(
			"Evening"
		);
		expect(getTimeBlockFromTimestamp("2025-01-01T03:59:59Z")).toBe(
			"Evening"
		);
	});
	...
	test("Test canDropEventInTimeBlock - No hard_start", () => {
		...
		expect(
			canDropEventInTimeBlock(event, "Afternoon", "2025-01-02", 1)
		).toBe(true);
	});
	...
	test("Test canDropEventInTimeBlock - Wrong time block", () => {
		...
		expect(
			canDropEventInTimeBlock(event, "Afternoon", "2025-01-01", 1)
		).toBe(false);
	});
	...
	test("Test getDropErrorMessage - Event with hard_start", () => {
		...
		expect(message).toBe(
			'"Concert" has a fixed start time and must be placed in the Evening block on 2025-01-01.'
		);
	});
